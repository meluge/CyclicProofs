...
The WF backend is fed by a phase-aware pre-pass
(\file{CyclicTactic/Phase.lean}, §\ref{sec:impl-wf}) applied to the labelled
size-change graphs before the SCT check. A finite-domain argument---a
\code{Bool} or enum whose value is a nullary constructor---can carry
well-founded progress that the structural extractor reads as stagnation, which
is exactly why the Sprenger-Dam two-step (Cyclist Test~13) fails the SCT gate
before any measure is synthesised. The pre-pass records each back-edge's
finite-domain constructor transition, requires the transition on every
\emph{stalled} back-edge (one with no structural strict self-loop) to strictly
decrease, solves a strict constructor order from those requirements by
topological ranking (rejecting the proof if they are cyclic), and patches a
strict self-loop into each stalled graph at the phase position. A proof whose
only progress is a finite-domain phase thereby becomes SCT-valid, and
\code{synthMeasure} renders the phase position as a total \code{match}-rank
component of the lexicographic measure. Because the augmentation only adds
edges---and runs after \code{canStructural} has consulted the raw
graphs---it can only widen the WF-emittable class, never reroute a structural
proof.
...
(Conjectures~\ref{conj:dispatch}.3 and~\ref{conj:lob}). Phase-aware
extraction (§\ref{sec:impl-wf}) enlarges the well-founded middle at the
expense of the remainder: a finite-domain phase proof that \code{synthMeasure}
alone leaves at \code{none} is, after augmentation, SCT-valid and discharged by
a lexicographic measure, so it falls under clause~2 rather than the Löb-only
case. Clauses~1 and~2
