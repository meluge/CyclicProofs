...
not handle and describes the \emph{dispatcher} that routes a non-structural
proof---most prominently merge-sort---to a well-founded backend driven by
Lee 2009-style measure synthesis~\cite{lee2009ranking}, extended with
phase-aware extraction so that finite-domain (Sprenger-Dam two-step) descent
is modelled as a size-change. It also outlines the one emission strategy still
under separate development: Löb-induction emission via a step-indexed
embedding~\cite{atkey2013productive}.
