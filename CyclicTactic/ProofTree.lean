...
nodes. The design mirrors the function side (`CyclicTactic.Extract`) but for
propositions rather than definitions.

## Scope (stage 2)

- Subject terms: variables and named constructors (no recursive calls,
  unlike `CyclicTactic.Extract.Term` which models function bodies).
...
/-- A term that occurs inside a formula — variables and constructors.
    Distinct from `CyclicTactic.Extract.Term` because proof terms do not have
    "recursive calls". -/
...
annotation pass (`CyclicTactic.Annotation`) to attribute progressing-name
